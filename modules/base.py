# coding=utf-8
# code adopted from huggingface
# base class of Config and CLIP4Clip Model
...
class PretrainedConfig(object):
    ...
    def get_config(cls, pretrained_model_name, cache_dir, type_vocab_size, state_dict, task_config=None):
        ...
        if resolved_archive_file == archive_file:
            if task_config is None or task_config.local_rank == 0:
                log_info("loading archive file {}".format(archive_file))
        else:
            if task_config is None or task_config.local_rank == 0:
                log_info("loading archive file {} from cache at {}".format(
        ...
        if os.path.isdir(resolved_archive_file):
            ...
            if task_config is None or task_config.local_rank == 0:
                log_info("extracting archive file {} to temp dir {}".format(
        ...
        if task_config is None or task_config.local_rank == 0:
            log_info("config of Cross Model {}".format(config))
        ...
        if state_dict is None:
            ...
            if os.path.exists(weights_path):
                ...
                if task_config is None or task_config.local_rank == 0:
                    log_info("Weight doesn't exsits. {}".format(weights_path))
    ...
    def init_preweight(cls, model, state_dict, prefix=None, task_config=None):
        ...
        if prefix is None and (task_config is None or task_config.local_rank == 0):
            ...
            if len(missing_keys) > 0:
                log_info("Weights of {} not initialized from pretrained model: {}"
                            .format(model.__class__.__name__, "\n   " + "\n   ".join(missing_keys)))
            if len(unexpected_keys) > 0:
                log_info("Weights from pretrained model not used in {}: {}"
                            .format(model.__class__.__name__, "\n   " + "\n   ".join(unexpected_keys)))
            if len(error_msgs) > 0:
                logging.error("Weights from pretrained model cause errors in {}: {}"
                             .format(model.__class__.__name__, "\n   " + "\n   ".join(error_msgs)))
