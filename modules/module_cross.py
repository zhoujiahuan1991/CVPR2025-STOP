# coding=utf-8
"""
An cross model to fuse the text feature and visual feature
"""
...
class CrossEmbeddings(nn.Module):
    ...
    def forward(self, concat_embeddings, position_ids=None, concat_type=None):
...
class CrossModel(PreTrainedModel):
    ...
    def __init__(self, config):
        """
        A cross model to fusion text features and visual features.
        Args:
            config: An instance of CrossConfig
        """
        super(CrossModel, self).__init__(config)

        self.embeddings = CrossEmbeddings(config)
        self.transformer = Transformer(width=config.hidden_size,
    ...
    def initialize_parameters(self):
        """initialize weights for {MultiheadAttention, MLP} in transformer"""
